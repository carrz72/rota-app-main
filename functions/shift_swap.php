...
if ($action === 'propose') {
    ...
    try {
        ...
        log_audit($conn, $_SESSION['user_id'] ?? null, 'shift_swap_proposed', ['from_user' => $from_user, 'to_user' => $to_user, 'from_shift' => $from_shift, 'to_shift' => $to_shift], $swapId, 'shift_swap', session_id());
    } catch (Exception $e) {
    }
...
if ($action === 'accept') {
    ...
    try {
        ...
        try {
            require_once __DIR__ . '/../includes/audit_log.php';
            log_audit($conn, $_SESSION['user_id'] ?? null, 'shift_swap_accepted', ['from_shift' => $swap['from_shift_id'], 'to_shift' => $swap['to_shift_id'], 'from_user' => $swap['from_user_id'], 'to_user' => $swap['to_user_id']], $swap_id, 'shift_swap', session_id());
        } catch (Exception $e) {
        }
    } catch (Throwable $e) {
        $conn->rollBack();
...
if ($action === 'decline') {
    ...
    try {
        require_once __DIR__ . '/../includes/audit_log.php';
        log_audit($conn, $_SESSION['user_id'] ?? null, 'shift_swap_declined', ['to_user' => $_SESSION['user_id']], $swap_id, 'shift_swap', session_id());
    } catch (Exception $e) {
    }
    if (!empty($_POST['notif_id'])) {
        $nid = (int) $_POST['notif_id'];
