...
<head>
    ...
    <style>
        ...
        .stat-label {
            ...
            padding: 20px;
        @media (max-width: 480px) {gba(0, 0, 0, 0.05);
            .quick-stats {
                grid-template-columns: 1fr;
            }board-card h3 {
        }   margin-top: 0;
            display: flex;
        .day-badge {ems: center;
            display: inline-block;
            width: 30px;
            height: 30px;600;
            line-height: 30px;
            text-align: center;x;
            border-radius: 50%;solid #eee;
...
<body>  }
    <div class="dashboard-container">
        ...
        <div class="welcome-card">
            <div class="welcome-text">eat(auto-fit, minmax(120px, 1fr));
                <h1>Welcome,
                    <?php echo !empty($_SESSION['username']) ? htmlspecialchars($_SESSION['username']) : 'Guest'; ?>
                </h1>
                <p><?php echo date("l, F j, Y"); ?> •
                    <?php echo !empty($_SESSION['role']) ? ucfirst(htmlspecialchars($_SESSION['role'])) : 'User'; ?></p>
            </div>y: flex;
        ...
        <div class="quick-stats">
            ...
            </div>ound-color: #f0f7ff;
            <div class="stat-card">#3498db;
        ...
        <div class="dashboard-card">
            ...
            <?php endif; ?>cho !empty($_SESSION['username']) ? htmlspecialchars($_SESSION['username']) : 'Guest'; ?>
        </div>  </h1>
                <p><?php echo date("l, F j, Y"); ?> •
        <!-- Upcoming Shifts Section -->SESSION['role']) ? ucfirst(htmlspecialchars($_SESSION['role'])) : 'User'; ?></p>
        <div class="dashboard-card" style="grid-column: 1 / -1;">
            <h3><i class="fas fa-calendar-alt"></i> Upcoming Shifts</h3>
                <a href="shifts.php"><i class="fas fa-calendar"></i> My Shifts</a>
            <?php if (!empty($next_shifts)): ?>s fa-users"></i> Rota</a>
                <table class="upcoming-shifts-table">fa-cog"></i> Settings</a>
                    <tr> ($_SESSION['role'] === 'admin'): ?>
                        <th>Date</th>/admin_dashboard.php"><i class="fas fa-shield-alt"></i> Admin</a>
                        <th>Time</th>
                        <th>Role</th>
                        <th>Location</th>
                        <th>Est. Pay</th>
        ...
        <div class="quick-stats">
                    <?phptat-card">
                    $current_date = '';"><?php echo $month_stats['shift_count'] ?? 0; ?></div>
                    foreach ($next_shifts as $shift):onth</div>
                        $shift_date = date("Y-m-d", strtotime($shift['shift_date']));
                        $is_new_date = ($shift_date != $current_date);
                        $current_date = $shift_date;$whole_hours; ?></div>
                <div class="stat-label">Hours This <?php echo ucfirst($period); ?></div>
                        // Format for display
                        $formattedShiftDate = date("D, M j, Y", strtotime($shift['shift_date']));
                        $formattedStartTime = date("g:i A", strtotime($shift['start_time']));iv>
                        $formattedEndTime = date("g:i A", strtotime($shift['end_time']));v>
                        ?>
                        <tr>-card">
                            <td><?php echo $formattedShiftDate; ?></td> + ($next_shift ? 1 : 0); ?></div>
                    ...
                    <?php endforeach; ?>
                </table>="fas fa-clock"></i> Next Shift</h3>

                <div style="margin-top: 15px; text-align: right;">
                    <a href="shifts.php" style="font-size: 0.9rem;">View all shifts <i class="fas fa-arrow-right"></i></a>
                </div>ttedDate = date("l, F j, Y", strtotime($next_shift['shift_date']));
            <?php else: ?>Start = date("g:i A", strtotime($next_shift['start_time']));
                ...
                </div> = new DateTime('today');
            <?php endif; ?> = new DateTime($next_shift['shift_date']);
        </div>  $days_until = $today->diff($shift_date)->days;
                $days_label = $days_until == 0 ? 'Today' : ($days_until == 1 ? 'Tomorrow' : "In $days_until days");
        <?php if ($_SESSION['role'] === 'admin'): ?>
            <!-- Admin Quick Access --> start datetime
            <div class="dashboard-card">-m-d H:i:s", strtotime($next_shift['shift_date'] . " " . $next_shift['start_time']));
                <h3><i class="fas fa-shield-alt"></i> Admin Tools</h3> day to the end datetime
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 15px;">
                    <a href="../functions/shift_invitation_sender.php" style="text-decoration: none;">xt_shift['end_time']));
                        <div style="background-color: #f9f9f9; padding: 15px; border-radius: 8px; text-align: center;">
                            <i class="fas fa-paper-plane" style="font-size: 2rem; color: #fd2b2b; margin-bottom: 10px;"></i>y")) . " " . $next_shift['end_time']));
                    ...
                    <a href="../admin/admin_dashboard.php" style="text-decoration: none;">
                        <div style="background-color: #f9f9f9; padding: 15px; border-radius: 8px; text-align: center;">
                            <i class="fas fa-tachometer-alt"ate("d", strtotime($next_shift['shift_date'])); ?></span>
                                style="font-size: 2rem; color: #fd2b2b; margin-bottom: 10px;"></i>
                            <p style="margin: 0; color: #333; font-weight: 500;">Admin Dashboard</p>/span>
                        </div>
                    </a>
                </div>iv class="next-shift-info">
            </div>      <div class="next-shift-meta">
        <?php endif; ?>     <i class="fas fa-clock"></i>
    </div>                  <span><?php echo $formattedStart; ?> - <?php echo $formattedEnd; ?></span>
                        </div>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Initialize the burger menu functionality "></i>
            const menuToggle = document.getElementById('menu-toggle');                     <span><?php echo htmlspecialchars($next_shift['role_name']); ?></span>
...
</html></body>    <script src="/rota-app-main/js/links.js"></script>    <script src="/rota-app-main/js/pwa-debug.js"></script>    <script src="/rota-app-main/js/menu.js"></script>    </script>        });            }                });                    this.form.submit();                periodSelect.addEventListener('change', function() {            if (periodSelect) {            const periodSelect = document.getElementById('period');            // Handle period selector                        document.querySelector('header').style.opacity = "1";            // Set header opacity to 1 once page is loaded                        }                });                    }                        navLinks.classList.remove('show');                        navLinks.classList.contains('show')) {                        !navLinks.contains(event.target) &&                     if (!menuToggle.contains(event.target) &&                 document.addEventListener('click', function(event) {                // Close menu when clicking outside                                });                    console.log('Menu toggled');                    navLinks.classList.toggle('show');                menuToggle.addEventListener('click', function() {            if (menuToggle && navLinks) {                        <div class="next-shift-meta">
                            <i class="fas fa-map-marker-alt"></i>
                            <span><?php echo htmlspecialchars($next_shift['location']); ?></span>
                        </div>
