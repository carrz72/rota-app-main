...
<head>
    ...
    <title>Retention Settings - Admin</title>
    <link rel="stylesheet" href="../css/admin_dashboard.css">
    <style>
        .panel { background:#fff; padding:16px; border:1px solid #e6e6e6; border-radius:6px; max-width:720px; }
        label { display:block; margin-top:8px; }
        input[type=text], input[type=number] { padding:8px; width:100%; box-sizing:border-box; border:1px solid #ddd; border-radius:4px; }
        .muted { color:#666; font-size:12px; }
        .btn { display:inline-block; padding:8px 12px; background:#2d6cdf; color:#fff; border-radius:4px; text-decoration:none; border:0; }
...
<body>
    <div class="admin-container">
        ...
        <p class="muted">Configure the scheduled admin id used by automated purge jobs and the audit retention period (days).</p>

        <div class="panel">
            <?php if (!empty($errors)): ?>
                <div style="color:#c0392b;"><strong>Errors:</strong><ul><?php foreach ($errors as $e) echo '<li>'.h($e).'</li>'; ?></ul></div>
            <?php endif; ?>
            <?php if ($success): ?>
            ...
            <form method="post" action="retention_settings.php">
                ...
                <input id="schedule_admin_id" name="schedule_admin_id" type="text" value="<?php echo h($currentScheduleAdmin); ?>" placeholder="Leave empty to disable">
                <div class="muted">This user id will be used to record automated archive/delete admin actions. Must be a valid admin user id.</div>

                <label for="audit_retention_days">Audit retention (days)</label>
                <input id="audit_retention_days" name="audit_retention_days" type="number" min="1" max="3650" value="<?php echo h($currentRetention); ?>">
                <div class="muted">Number of days to keep audit rows before automated archival. Limits: 1 - 3650 (10 years).</div>
            ...
            <p class="muted">This shows the optional scheduler trigger stored in `app_settings` under the key <code>SCHEDULE_TRIGGER</code>. Editing the OS scheduled task must be done via Task Scheduler or PowerShell (instructions below).</p>
            <div style="background:#f7f7f7;padding:10px;border:1px solid #eee;border-radius:4px;">
                <strong>Stored trigger:</strong>
                <div style="margin-top:8px;font-family:monospace;"><?php echo h($currentTrigger === '' ? '(not set)' : $currentTrigger); ?></div>
            </div>

            <h4 style="margin-top:12px;">PowerShell commands (Windows)</h4>
            <p class="muted">Use these from an elevated PowerShell prompt to inspect or register the scheduled task that runs the purge script. Adjust paths and task name as needed.</p>
