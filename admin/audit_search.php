...
<head>
    <?php $PAGE_TITLE = 'Audit Search - Admin';
...
<body>
    <div class="admin-container">
        <h1>Audit Search</h1>
        <p class="muted">Search the audit log. Use the filters below to narrow results. Results are limited and
            paginated.</p>
            ...
            <form method="get" action="audit_search.php">
                <div class="row">
                    <div class="col">
                        ...
                        <input id="user" name="user" type="text" placeholder="Enter user id or username (partial)"
                            value="<?php echo h($filters['user']); ?>">
                    </div>
                    ...
                    <div class="col">
                        ...
                        <input id="session_id" name="session_id" type="text" placeholder="Partial session id"
                            value="<?php echo h($filters['session_id']); ?>">
                    </div>
        ...
        <div class="results-head">
            ...
            <div style="display:flex; gap:12px; align-items:center;">
                ...
                <button id="anonymiseBtn" type="button"
                    style="background:#c0392b;color:#fff;padding:8px;border-radius:4px;border:0;">Anonymise selected /
                    by filter</button>
                <!-- Delete all logs (stronger gating) -->
                <button id="deleteAllBtn" type="button"
                    style="background:#7f1d1d;color:#fff;padding:8px;border-radius:4px;border:0;">Delete all
                    logs</button>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table>
                ...
                <tbody>
                    <?php if (empty($rows)): ?>
                        <tr>
                            <td colspan="7">No audit events found for these filters.</td>
                        </tr>
                    <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td class="meta">
                                    ...
                                    <?php if (strlen($r['meta']) > 120): ?>
                                        <button type="button" class="view-meta"
                                            data-meta="<?php echo h(addslashes($r['meta'])); ?>">View</button>
                                    <?php endif; ?>
                                </td>
...
<div id="metaModal"
    ...
    <div
        style="background:#fff; max-width:900px; width:90%; max-height:80%; overflow:auto; padding:16px; border-radius:6px; position:relative;">
        <button id="metaClose" style="position:absolute; right:8px; top:8px;">Close</button>
        <h3>Meta details</h3>
        <pre id="metaContent"
            style="white-space:pre-wrap; font-family:monospace; background:#f7f7f7; padding:12px; border:1px solid #eee;"></pre>
    </div>
</div>
...
<div id="anonymiseModal"
    ...
    <div style="background:#fff; max-width:700px; width:90%; padding:16px; border-radius:6px; position:relative;">
        ...
        <p class="muted">This will remove personal identifiers (user_id, ip address, user agent, and common PII keys in
            meta) for matching audit rows. A dry-run will show how many rows will be affected.</p>
        ...
        <div style="margin-top:12px;">
            <label><input type="checkbox" id="allowAll"> Allow all (dangerous) - operate on all audit rows matching no
                filter</label>
        </div>

        <div style="margin-top:12px; display:flex; gap:8px;">
            <button id="runDry" type="button">Run</button>
            <button id="confirmRun" type="button" disabled
                style="background:#c0392b;color:#fff;border:0;padding:8px;border-radius:4px;">Confirm</button>
        </div>
        <div style="margin-top:12px;">
            <label for="erase_code">6-digit Erase Code (required for irreversible archive+delete)</label>
            <input id="erase_code" type="text" inputmode="numeric" pattern="\d{6}" placeholder="123456" maxlength="6"
                style="width:100%; max-width:200px;" />
            <div class="muted">Each admin who can delete logs may set their personal 6-digit erase code in their
                profile. If not set, your admin password will be accepted as a fallback.</div>
        </div>
        <?php $csrf_token_an = generate_csrf_token(); ?>
        <input type="hidden" id="csrf_token_field" name="csrf_token"
            value="<?php echo htmlentities($csrf_token_an); ?>">
        <div id="modeWarning" style="margin-top:12px; color:#a33; display:none; font-weight:bold;">Archive+delete is
            irreversible: encrypted archive will be created, originals will be deleted. Ensure you have backups and the
            encryption key is securely stored.</div>
...
<div id="deleteAllModal"
    ...
    <div style="background:#fff; max-width:700px; width:90%; padding:16px; border-radius:6px; position:relative;">
        ...
        <p class="muted">This will archive (encrypted) and then permanently delete audit log rows matching the current
            filters or all rows if no filter is set. This action is irreversible and will be recorded in
            `audit_admin_actions` with a signed proof. Use the dry-run first.</p>
        ...
        <div style="margin-top:12px;">
            <label><input type="checkbox" id="chk_backup"> I have a verified backup of the database
                (recommended)</label>
            <br>
            <label><input type="checkbox" id="chk_irrevocable"> I understand this action is irreversible and will
                permanently delete the selected audit logs</label>
            <br>
            <label><input type="checkbox" id="deleteAllowAll"> Allow all (dangerous) - operate on all audit rows
                matching no filter</label>
        </div>

        <div style="margin-top:12px;">
            <label for="erase_code_delete">6-digit Erase Code (required)</label>
            <input id="erase_code_delete" type="text" inputmode="numeric" pattern="\d{6}" placeholder="123456"
                maxlength="6" style="width:100%; max-width:200px;" />
            <div class="muted">If you don't have an erase code set, you may enter your admin password below as a
                fallback.</div>
        </div>

        <div style="margin-top:12px;">
            <label for="confirm_admin_password_delete">Admin password (fallback)</label>
            <input id="confirm_admin_password_delete" type="password" placeholder="Enter your admin password (fallback)"
                style="width:100%; max-width:420px;" />
        </div>

        <div style="margin-top:12px; display:flex; gap:8px;">
            <button id="runDeleteDry" type="button">Run dry-run</button>
            <button id="confirmDeleteAll" type="button" disabled
                style="background:#c0392b;color:#fff;border:0;padding:8px;border-radius:4px;">Confirm Delete
                All</button>
        </div>
        ...
        <input type="hidden" id="csrf_token_delete" name="csrf_token"
            value="<?php echo htmlentities($csrf_token_del); ?>">
    </div>
</div>
